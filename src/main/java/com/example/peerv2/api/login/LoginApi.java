...
import com.example.peerv2.exceptionHandler.ConflictTest;
...
//    public UserDto findMember(@PathVariable int user_num) {
//        UserDto userDto = loginService.findMember(user_num);
//
//        if (userDto == null) {
...
//    }
    ...
    @GetMapping("/findmember/{user_num}")
    public ResponseEntity<UserDto> findMember(@PathVariable int user_num) {
        UserDto userDto = loginService.findMember(user_num);
        if(userDto == null){
    ...
    public ResponseEntity<Object> findMember(@PathVariable int user_num) {
        ...
        if (userDto.getUser_age() > 20) {
            throw new ConflictTest(String.format("conflict"));
        }
